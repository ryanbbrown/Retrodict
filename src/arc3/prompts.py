...
- Explore, then commit: while an action's effect is still uncertain, probe with a single action carrying an `expect`; once a probe confirms how it behaves, stop re-probing and commit a multi-action plan toward the goal with an `expect` on every action. Advancing a mechanic you already understand one action per reply — a fresh call for each single click — is the most common way a run stalls, so batch every move whose result you can already predict. If an action produced no change or a bad outcome, do not simply repeat it; form a new hypothesis for why the result would differ before spending another action on it.
