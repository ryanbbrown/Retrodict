...
Retrodict is an agent for [ARC-AGI-3](https://three.arcprize.org) that completes 21/25 games and scores a mean RHAE of 84.52% on an [official competition-mode scorecard](https://arcprize.org/scorecards/8d734689-3eb9-4dee-b0ce-d822d76e0689), the highest publicly reported score, in a total cost of $418.71. Built on [ThinHarness](https://github.com/ryanbbrown/thinharness); run with `openai:gpt-5.6-sol` at high reasoning effort.
...
The bigger gap is tokens. On the levels each agent actually solved, I spent **452M tokens vs 5,124M for secure_baseline_v1.5**, about 11× less. Including unsolved levels results in a 17× difference (518M vs 9,082M), but that's mostly a comparison of cap policy and not as representative. Both sides are counted the same way (billed input + output) from each run's own logs, split per level. Per-game breakdowns: [mine](docs/per-game-level-costs.md), [baseline1's](docs/baseline1-per-game-level-costs.md). Both models bill at the same API rates ($5.00 input / $0.50 cached / $30.00 output per MTok), so the cost comparison translates roughly: $372 vs $3,572 on solved levels. However, baseline1 appears to have run through a Codex subscription, so it's likely they actually paid much less.

The score comparison is also not apples to apples: gpt-5.6 sol is a more powerful model than gpt-5.5, and baseline1 published one run per game, while two of my 25 games use a re-run (no harness or prompt changes between attempts), and lf52 was re-run live for the scorecard (see [Validity](#validity)):
