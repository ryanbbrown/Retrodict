...
5. Re-invoke the agent — passing the persisted `HarnessResult.resume_state` as `resume_from` so the conversation continues — when: queue empty, `levels_completed` changes, `state` transitions, or a mid-drain validation mismatch. On `GAME_OVER`, the runner issues `RESET` (an attempt/level reset — the same protocol baseline1's controller uses after death; whole-game restarts are never used) and tells the agent. Start a fresh conversation (no `resume_from`) when the conversation's context — the input tokens of the invocation's final provider request — exceeds 150k; the log is the durable memory, and the fresh-session prompt directs the agent to re-read `log.txt`. (Cumulative `RunUsage.input_tokens` is the wrong signal: it re-counts the cached prefix once per tool round, so one tool-heavy invocation "exceeds" 150k while the actual conversation is ~25k.)
...
- **Session continuity**: continue one conversation per game via `resume_state`/`resume_from` until the conversation context reaches the 150k-token threshold, then fresh session. Alternative (fresh every invocation) is simpler and more cache-friendly but discards in-context reasoning; revisit with pilot data.
...
- **Phase A — port calibration** (games with published RGB + baseline1 + human numbers), run in order ft09 → ls20 → vc33. ft09 goes first: cheapest (RGB finished it in 78 actions), the strongest signal for the recipe (RGB's agent beat the human playback there via an analytic Lights-Out solve in python — if the port can't do respectably here, the port is broken), and it continues directly from step 7's API smoke run. Success: solve ft09 and ls20 fully within ~2× RGB's action counts (≤160 / ≤1,100). Context: clean baseline1 solved both but inefficiently (ft09 57.8% RHAE at 474 steps; ls20 57.0% at 1,600). vc33 is informative either way — clean baseline1 reached only 3/7; RGB solved it.
